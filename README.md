...
| ------- | ------------- | ----- |
| Callback never arrives | `PUBLIC_CALLBACK_URL` value; ngrok running | `.env` |
| Wrong base/endpoint URL | Endpoint configuration | `config.py` |
| Where the 3 route URLs are built | `successRedirectionUrl`, `failureRedirectionUrl`, `callbackApiUrl` | `services/onekhusa_service.py` |
| Webhook route missing / wrong method | Route registration (`POST /api/webhooks/payments`) | `app.py` |
| Webhook received but status not updating | Handler logic (`is_success` → `Paid`, ack 200) | `app.py` |
| Page not reflecting a processed payment | Frontend polling logic (`checkPaymentStatus`) | `templates/welcome.html` |
| Need the full request history | Console logs (`OneKhusa webhook received`, `status updated to PAID/FAILED`) | Terminal running `python app.py` |

**Fastest check:** look for `OneKhusa webhook received` in the app console.

- **Not there** → OneKhusa never reached your callback. Verify ngrok, `.env`, and the route.
- **There, but customer still saw failure** → confirm the handler returned `HTTP 200 OK` (it does unless an exception is thrown).
...
- **Rotate** `ONEKHUSA_API_KEY` / `ONEKHUSA_API_SECRET` if they're ever exposed.
- **Production:** persist transaction status to a database instead of the in-memory cache, and add webhook signature verification if provided by OneKhusa.
